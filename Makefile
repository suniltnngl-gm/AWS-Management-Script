...
.PHONY: all test analyze build deploy clean log-rotate help
...
log-rotate: ## Rotates and compresses the main log file
@echo "--- Rotating log file: $(LOG_FILE) ---"
@if [ -f "$(LOG_FILE)" ]; then \
"$(LOG_FILE)-$$timestamp"; \
to $(LOG_FILE)-$$timestamp.gz"; \
else \
ot found. Nothing to rotate."; \
fi
